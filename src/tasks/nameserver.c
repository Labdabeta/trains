...
void name_server(){
  ...
  /*debugio_putstr("Entering nameserver\n\r");
  int mytid = MyTid();
  debugio_putuint_decimal(mytid);
  debugio_putstr("\n\r");*/

  volatile char names[MAX_NAME_LENGTH * NAMES_CAPACITY];
  volatile char tids[NAMES_CAPACITY];
  volatile int space[100];
  volatile char msg[MAX_NAME_LENGTH];
  volatile int size = 0;
  volatile int tid, err;

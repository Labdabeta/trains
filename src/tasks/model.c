...
ENTRY initialize(struct Data *data)
{
	...
	i = CreateSize(1, A0_initialization, TASK_SIZE_SMALL);
	Send(i, 0, 0, 0, 0); //Wait for the initial screen to print

	data->echo_tid = CreateSize(1, echo_view, TASK_SIZE_SMALL);
	data->flip_tid = CreateSize(1, flip_view, TASK_SIZE_SMALL);
	data->sensor_tid = CreateSize(1, sensor_view, TASK_SIZE_SMALL);
}

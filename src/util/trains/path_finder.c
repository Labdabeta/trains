...
{
	int offset = pathAB->length - 1;
	int ABdist = pathAB->distances[offset];
	pathAB->length = pathAB->length + pathBC->length - 1;
	pathAB->trueLength = pathAB->trueLength + pathBC->trueLength - 1;
	for(int index = 1; index < pathBC->length; index++){
		pathAB->sensors[index + offset] = pathBC->sensors[index];
		pathAB->distances[index + offset] = ABdist + pathBC->distances[index];
		pathAB->states[index + offset] = pathBC->states[index];
